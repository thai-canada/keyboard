...
Sub injectCode(m as object)
	...
	if type(m.htmlWidget) = "roHtmlWidget" then
		...
		if type(m.htmlWidget) = "roHtmlWidget" then
			...
			jsCode$ = jsCode$ + "" + Chr(10)
			jsCode$ = jsCode$ + "                if (key === 'Backspace' || key === 'Caps' || key === 'Close' || key === 'Enter') {" + Chr(10)
			...
			jsCode$ = jsCode$ + "                    break;" + Chr(10)
			jsCode$ = jsCode$ + "                case 'enter':" + Chr(10)
			jsCode$ = jsCode$ + "                    if (activeInput.tagName === 'TEXTAREA') {" + Chr(10)
			jsCode$ = jsCode$ + "                        activeInput.value += '\\n';" + Chr(10)
			jsCode$ = jsCode$ + "                    } else if (activeInput.form && typeof activeInput.form.submit === 'function') {" + Chr(10)
			jsCode$ = jsCode$ + "                        activeInput.form.submit();" + Chr(10)
			jsCode$ = jsCode$ + "                    } else {" + Chr(10)
			...
			jsCode$ = jsCode$ + "        function emitKeySequence(key) {" + Chr(10)
			jsCode$ = jsCode$ + "            const keyName = key === ' ' ? 'Space' : key;" + Chr(10)
			jsCode$ = jsCode$ + "            const down = new KeyboardEvent('keydown', { key: keyName, bubbles: true, cancelable: true });" + Chr(10)
			jsCode$ = jsCode$ + "            const press = new KeyboardEvent('keypress', { key: keyName, bubbles: true, cancelable: true });" + Chr(10)
			jsCode$ = jsCode$ + "            const up = new KeyboardEvent('keyup', { key: keyName, bubbles: true, cancelable: true });" + Chr(10)
			jsCode$ = jsCode$ + "            activeInput.dispatchEvent(down);" + Chr(10)
			jsCode$ = jsCode$ + "            activeInput.dispatchEvent(press);" + Chr(10)
			jsCode$ = jsCode$ + "            activeInput.dispatchEvent(up);" + Chr(10)
			jsCode$ = jsCode$ + "        }" + Chr(10)
			jsCode$ = jsCode$ + "        if (action === 'backspace') {" + Chr(10)
			jsCode$ = jsCode$ + "            emitKeySequence('Backspace');" + Chr(10)
			jsCode$ = jsCode$ + "        } else if (action === 'space') {" + Chr(10)
			jsCode$ = jsCode$ + "            emitKeySequence(' ');" + Chr(10)
			jsCode$ = jsCode$ + "        } else if (value) {" + Chr(10)
			jsCode$ = jsCode$ + "            emitKeySequence(value);" + Chr(10)
			jsCode$ = jsCode$ + "        }" + Chr(10)
